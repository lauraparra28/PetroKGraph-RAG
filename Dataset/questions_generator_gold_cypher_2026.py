...
print("✅ Successfully loaded PetroKGraph merged.")
...
def gen_questions(fields, basins, wells, formations, graph, random_sample=False):
    ...
    # 2) FIELDS
    # ===============================================================
    print("🔹 Generating FIELD questions...")
    for field_uri, info in fields.items():
        ...
        if field_names:
            ...
            aggregation_questions.append({
                    ...
                    "gold_cypher": (f"MATCH (f:field) WHERE '{main_name}' IN f.rdfs_label RETURN f.rdfs_label")
            })
    ...
    print("🔹 Generating FIELD count and list questions by basin...")
    ...
    print("🔹 Generating WELL questions...")
    ...
    for well_uri, info in wells.items():
        ...
        if well_names:
            ...
            aggregation_questions.append({
                    ...
                    "answer": answer_text,
                    "context": context_text,
                    "gold_cypher": (f"MATCH (w:well) WHERE '{main_name}' IN w.rdfs_label RETURN w.rdfs_label")
            })
    ...
    print("🔹 Generating FORMATION questions...")
    ...
    pozos_por_bacia = {}
    print("🔹 Generating WELL count questions by basin...")
    ...
    for rock_uri, (rock_label, rock_definition) in rock_data.items():
        ...
        def_questions.append({
            ...
            "gold_cypher": f"MATCH (r:Class) WHERE ANY(label IN r.rdfs_label WHERE split(label,'@')[0] = '{rock_label}') WITH [x IN r.definition WHERE x ENDS WITH '@pt'] AS def_pt RETURN def_pt",
...
with open("PetroKGraph_dataset_gold_cypher_balanced_complete.json", "w", encoding='utf-8') as f:
    json.dump(questions_balanced, f, ensure_ascii=False, indent=4)
print(f"✅ Saved PetroKGraph {len(questions_balanced)} balanced questions with gold answers.")  

with open("PetroKGraph_dataset_gold_cypher_unbalanced_complete.json", "w", encoding='utf-8') as f:
    json.dump(questions_unbalanced, f, ensure_ascii=False, indent=4)
print(f"✅ Saved PetroKGraph {len(questions_unbalanced)} unbalanced questions with gold answers.")  
...
output_path = os.path.join(data_dir, "PetroKGraph_dataset_gold_cypher_aggregation.json")
with open(output_path, "w", encoding='utf-8') as f:
    json.dump(questions_balanced_level0, f, ensure_ascii=False, indent=4)
print(f"✅ Saved PetroKGraph {len(questions_balanced_level0)} aggregation questions gold answers.")  


output_path_1hop = os.path.join(data_dir, "PetroKGraph_dataset_gold_cypher_one_hop.json")
with open(output_path_1hop, "w", encoding='utf-8') as f:
    json.dump(questions_balanced_one_hop, f, ensure_ascii=False, indent=4)
print(f"✅ Saved PetroKGraph {len(questions_balanced_one_hop)} one-hop questions gold answers.")

output_path_multi_hop = os.path.join(data_dir, "PetroKGraph_dataset_gold_cypher_multi_hop.json")
...
print(f"✅ Saved PetroKGraph {len(questions_balanced_multi_hop)} multi-hop questions gold answers.")

output_path_definition = os.path.join(data_dir, "PetroKGraph_dataset_gold_cypher_definition.json")
...
print(f"✅ Saved PetroKGraph {len(questions_balanced_definition)} definition questions gold answers.")
